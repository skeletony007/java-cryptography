...
/**
 * A concrete subclass of <code>Substitution</code> with methods relating to
 * monoalphabetical substitution ciphers.
 * <p>
 * Monoalphabetical substitution ciphers iteratively replace each character in a
 * text according to onefixed substitution
 * table.<br>
 * This subclass overloads the two abstract methods <code>encrypt(char)</code>
 * and <code>decrypt(char)</code> from
 * <code>Substitution.java</code>.
 * <p>
 * Usage: <code>java MonoAlphaSubstitution encrypt key "cipher text"</code><br>
 * where <code>key</code> is the mapping where every character at an odd
 * position is the encoding of the one
 * directly before it<br>
 * and <code>"cipher text"</code> is the text to be encoded or decoded
 * <p>
 * Example usage:
 * <code>java MonoAlphaSubstitution encrypt akbjcidhegffgehdicjbka "Life is wasted on the living."</code>
 *
 * @see Substitution
 */
public class MonoAlphaSubstitution extends Substitution {
     ...
     *
     */
    public BiMap<Character, Character> substitutionTable;
    ...
    public MonoAlphaSubstitution(String mapping) {
        ...
        substitutionTable = new BiMap<>(mappingLength / 2);
    ...
    public char encrypt(char c) {
        try {
            return substitutionTable.get(c);
        } catch (Exception e) {
            return c;
        }
    }
     ...
     * <p>
     * Note that this method assumes that <code>substitutionTable</code> contains
     * only one-to-one mappings.
     *
    ...
    public char decrypt(char c) {
        try {
            return substitutionTable.inverse(c);
        } catch (Exception e) {
            return c;
        }
    }
     ...
     * The second command line argument is the key such that every character at an
     * odd position is the encoding of
     * the one directly before it.<br>
     * The third command line argument is the text to be encoded or decoded.
     *
     * @param args command line arguments specifying to encrypt/decrypt, key, and
     *             cipher text
     */
